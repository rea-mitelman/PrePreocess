...
for ifile=1:length(findx)
    if exist(handles.flist(findx(ifile)).fnm);%!!!
		...
		load(handles.flist(findx(ifile)).fnm,'upsamp_spk_shapes','samplingInterval','fold');
		extracts=[extracts upsamp_spk_shapes];


	end
end
handles.n_samp=size(upsamp_spk_shapes,1);
handles.Fs=fold/samplingInterval/1e3;
...
handles.trash=[-inf*ones(handles.n_samp,1) inf*ones(handles.n_samp,1)];
...
handles.Templates(1:3,:)=zeros(3,handles.n_samp);
handles.output = hObject;
handles.time=[0:handles.n_samp-1]/handles.Fs;
...
try
    plot(handles.time,handles.extracts(:,handles.alloc_index==i)','k'); axis tight
    hold on, plot(handles.time, mean(handles.extracts(:,handles.alloc_index==i)')','r')
...
% 	xx=0:1/3:65;
	C=zeros(handles.n_samp);
	for i=1:3
		% 		csT=csaps(1:64,handles.Templates(i,:));
		% 		TE=fnval(csT,xx);
		TE=handles.Templates(i,:);
		C=C+TE'*TE;
	end
	%     [V,D]=eig(C);
...
i=get(handles.templateList,'value');
handles.active(i)=0;
handles.T(:,:,i)=[-inf*ones(handles.n_samp,1) inf*ones(handles.n_samp,1)];
handles.alloc_index(handles.alloc_index==i)=0;
handles.Templates(i,:)=zeros(1,handles.n_samp);
guidata(hObject, handles);
...
axes(handles.ExtractsPlot), hold off
if ~isempty(S0)
    %plot(handles.time,handles.extracts(:,candidates(r)),'k');
    plot(handles.time,handles.extracts(:,S0),'k'); hold on
end
if ~isempty(S1)
    plot(handles.time,handles.extracts(:,S1),'b'); hold on
end
if ~isempty(S2)
    plot(handles.time,handles.extracts(:,S2),'r'); hold on
end
if ~isempty(S3)
    plot(handles.time,handles.extracts(:,S3),'g'); hold on
end
if ~isempty(S4)
    plot(handles.time,handles.extracts(:,S4),'y'); hold on
end
xlim([1,handles.n_samp])
axis auto
...
xxx=samples'*handles.PC(:,1);
yyy=samples'*handles.PC(:,2);

%     xxx2=samples'*pcvec(4:3:end-1,1);
%     yyy2=samples'*pcvec(4:3:end-1,2);

axes(handles.PCs_vectors), cla,
% plot(handles.time, handles.PC(4:3:end-1,1), handles.time, handles.PC(4:3:end-1,2),'r',handles.time,handles.PC(4:3:end-1,3),'g'), pause(3)
plot(handles.time, handles.PC(:,1), handles.time, handles.PC(:,2),'r',handles.time,handles.PC(:,3),'g'), pause(3)
...
% [~,~,v]=svds(ext_spl,3);
[~,~,v]=svds(handles.extracts(:,handles.alloc_index~=4)',3);
handles.PC=v;
handles.PCtype='PCA';
plot_pcs(handles)
...
[A,~]=fastica (handles.extracts(:,handles.alloc_index~=4), 'only','all','firstEig',1,'lastEig',50,'numOfIC',3, 'displayMode', 'off', 'verbose', 'off');
handles.PC=A;
handles.PCtype='ICA';
plot_pcs(handles)
