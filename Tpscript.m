        ...
        for Fn=1:Nfiles  
            ...
            try
                ...
                if strcmp(overwrite_mode,'overwrite') | ~ExistFlags(Fn,En)
                    ...
                    upsamp_Fs=100;
					[quitProg] = Tpprep(full_inFiles{Fn},MPFfiles{Fn,En},pcvec,thisChan,length(thisChan),upsamp_Fs); 
                end
                if quitProg, return, end
            catch       
                disp([  'Projecion to file :'  MPFfiles{Fn,En}  ' --> Could not be completed.']);
