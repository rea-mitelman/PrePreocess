function TH_mat=get_thersh_per_subsess(home_dir,day_path,n_std,n_elects,do_art_rem)
	...
	for u=1:n_elects
		...
		for i_file=all_ss{i_ss}
			...
			if any(strcmp(who('-file',full_file_name),unit_names{u}))
				...
				if do_art_rem
					str2eval=['remove_stim_artifact(' unit_names{u} ' , ' unit_names{u} '_KHz , stim_times);' ];
				else 
					str2eval = unit_names{u};
				end
				data_vec=[data_vec , eval(str2eval )];
